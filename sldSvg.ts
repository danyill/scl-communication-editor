...
function renderLabel(element: Element) {
  let deg = 0;
  let text: string | null | TemplateResult<2>[] = element.getAttribute('name');
  let weight = 400;
  let color = 'black';
  ...
  const id = element.tagName !== 'Text' ? identity(element) : nothing;
...
function renderIed(ied: Element) {
  const [x, y] = renderedPosition(ied);
...
export function sldSvg(substation: Element, gridSize: number): TemplateResult {
  ...
  >
    ...
    ${Array.from(substation.ownerDocument.querySelectorAll(':root > IED'))
      .filter(child => child.tagName === 'IED' && hasIedCoordinates(child))
      .map(ied => renderIed(ied))}
    ${Array.from(substation.ownerDocument.querySelectorAll(':root > IED'))
      .filter(child => child.tagName === 'IED' && hasIedCoordinates(child))
      .map(ied => renderLabel(ied))}
    ${Array.from(
